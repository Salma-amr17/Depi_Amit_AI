...
try:
    # NOTE: Ensure visuals.py is in the same directory
    import visuals as vs
except ImportError:
    st.error("Error: Could not import visuals.py. Please ensure the file is saved in the same directory.")
    st.stop()
...
@st.cache_data
def load_data():
    """Loads, cleans, prepares data, and runs PCA/Clustering."""
    try:
        # Assuming the 'customers.csv' file is accessible
        df = pd.read_csv(r'C:\Depi_Amit_AI\MachineLearning\P3-\customers.csv')
    ...
    data_for_ml = df[SPENDING_COLS]
    
    # 1. Log Transformation (to normalize skewed data)
    log_data = np.log(data_for_ml)
    ...
    # 4. K-Means Clustering (Using 5 Clusters, common for this dataset)
    n_clusters = 5
    kmeans = KMeans(n_clusters=n_clusters, random_state=42, n_init='auto')
    preds = kmeans.fit_predict(reduced_data)
    centers = kmeans.cluster_centers_

    # Return new results for ML visualizations
    return df, q99, channel_agg, region_agg, multi_cat_df, reduced_df, preds, centers, pca

df, q99, channel_agg, region_agg, multi_cat_df, reduced_df, preds, centers, pca = load_data()

# Check if data loaded successfully (df.empty handles FileNotFoundError case)
if df.empty or q99 is None:
    st.stop()

# --- 21 VISUALIZATION FUNCTIONS (using Matplotlib/Seaborn and visuals.py) ---
...
# The old viz_20_ecdf_plot and viz_18_grouped_bar_chart were replaced with new functions
# to make space for the clustering plot and avoid a naming conflict.
...
# --- NEW CLUSTERING VISUALIZATION (Chart 21) ---

# Note: We pass reduced_df (PCA results), preds (cluster labels), and centers (cluster centers)
def viz_21_cluster_plot(reduced_data, preds, centers):
    """21. Cluster Visualization - K-Means Clustering on PCA-Reduced Data"""
    
    # We use a dummy for pca_samples as it is not strictly required for the basic plot
    pca_samples_dummy = np.array([[0, 0]]) 
    
    # Call the visualization function from visuals.py
    vs.cluster_results(reduced_data, preds, centers, pca_samples_dummy)
    
    # Set the title explicitly for display context
    plt.title("21. Cluster Visualization: K-Means Clustering (5 Clusters) on PCA-Reduced Data", fontsize=16)
    
    return plt.gcf() # Return current Matplotlib figure

# List of all visualization functions for display (Now 21 charts)
# NOTE: The cluster plot is added to the end.
VIZ_FUNCTIONS = [
    ...
    # --- NEW CLUSTERING CHART ---
    (viz_21_cluster_plot, (reduced_df, preds, centers), "Clustering & ML Results"),
]
...
st.title("📊 21-Chart Wholesale Customer Segmentation Analysis")
st.markdown("This dashboard presents *21 unique visualizations* covering customer spending distribution, segment comparisons, and internal feature correlations, including **K-Means Clustering**.")
...
with st.sidebar:
    ...
    # Display PCA Explained Variance Ratios
    if pca is not None:
        variance_ratio = pd.DataFrame(pca.explained_variance_ratio_ * 100, index=['Dimension 1', 'Dimension 2'], columns=['Variance (%)'])
        st.subheader("PCA Results")
        st.dataframe(variance_ratio.T, use_container_width=True)
...
st.subheader(f"Charts ({len(charts_to_display)} displayed under '{selected_group}')")
st.divider()
...
for i in range(0, len(charts_to_display), 2):
    ...
    def safe_display_chart(col, chart_tuple):
        ...
        # Extract title from docstring (first line after the function name)
        title = func.__doc__.split(' - ')[1].strip() if func.__doc__ else func.__name__

        with col:
            with st.expander(f"{title}", expanded=True):
                try:
                    # Handle functions that take tuple of arguments
