# app.py - Cardiovascular Risk Dashboard
...
<style>
    ...
    .css-1d391kg {padding: 1rem 1rem 2rem;}
    h1, h2, h3 {font-family: 'Montserrat', sans-serif; color: #ff6b6b;}
    .stMetric {
    ...
    .stMetric > div {color: #ff6b6b; font-weight: bold;}
    .sidebar .css-1d391kg {background: rgba(0,0,0,0.3);}
...
@st.cache_data
def load_data():
    df = pd.read_csv("D:/DEPI_F/final/Cardiovascular Diseases Risk Prediction Dataset export 2025-10-15 21-12-56.csv")
    return df
...
smoking_filter = st.sidebar.multiselect(
    "Smoking History", options=df['Smoking_History'].unique(), default=df['Smoking_History'].unique()
)
...
with col1:
    st.markdown("<h1 class='pulse'>Heart</h1>", unsafe_allow_html=True)
with col2:
    st.markdown("<h3 style='margin-top: 1.5rem; color:#ff9f9f;'>Cardiovascular Risk Dashboard</h3>", unsafe_allow_html=True)
...
with tab1:
    ...
    with col1:
        ...
        color_map = {age: colors[i] for i, age in enumerate(age_order)}

        fig_age = go.Figure()
        fig_age.add_trace(go.Bar(
            x=age_counts.index,
            y=age_counts.values,
            marker_color=[color_map[age] for age in age_counts.index],
    ...
    with col2:
        ...
        sex_heart_pct = (
            df_filtered.groupby('Sex')['Heart_Disease'].value_counts(normalize=True).unstack()
            .get('Yes', pd.Series([0]*len(sex_counts))) * 100
        ).round(1)
...
<div style='text-align: center; color: #666; font-size: 0.9rem;'>
    <p>© 2025 Cardiovascular Risk Dashboard | Data: 308,854 Records</p>
</div>
""", unsafe_allow_html=True)